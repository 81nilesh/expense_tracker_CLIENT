...
const Main = () => {
  ...
  useEffect(() => {
    const getFromDB = async () => {
      try {
        ...
        const response = await axios.get('https://expenseapimac.onrender.com/expenses/getExpenses', { headers: { "Authorization": token } })
  ...
  const downloadCSV = async (e) => {
    ...
    try {
      const token = JSON.parse(localStorage.getItem('token'));
      let response = await axios.get('https://expenseapimac.onrender.com/expenses/downloadCSV', { headers: { "Authorization": token } });
